...
function getWebGLContext (canvas:HTMLCanvasElement) {
    ...
    let halfFloat:any | OES_texture_half_float;
...
function getSupportedFormat(gl:WebGL2RenderingContext, internalFormat:number, format:number, type:number) : null | {}
{
...
function splat (x:number, y:number, dx:number, dy:number, color:Array<number>) {
    splatProgram.bind();
    //@ts-expect-error
